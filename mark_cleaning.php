<?php
// mark_cleaning.php - set room status = CLEANING and add a cleaning_logs entry
require_once 'db.php';
session_start();
header('Content-Type: application/json');

if ($_SERVER['REQUEST_METHOD'] !== 'POST') { http_response_code(405); echo json_encode(['success'=>false,'error'=>'Invalid request method']); exit; }
if (!isset($_SESSION['user_id']) || !isset($_SESSION['role_id']) || !in_array(intval($_SESSION['role_id']), [1,2])) { http_response_code(403); echo json_encode(['success'=>false,'error'=>'Forbidden']); exit; }
