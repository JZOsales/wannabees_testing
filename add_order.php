...
try {
    ...
    $new_grand = round(floatval($bill['total_room_cost']) + $new_orders, 2);
    ...
    echo json_encode(['success'=>true,'order_id'=>$order_id,'order_total'=>round($orderTotal,2),'bill_id'=>$bill_id,'total_orders'=>$new_orders,'grand_total'=>$new_grand]);
    exit;
} catch (Exception $e) {
    $mysqli->rollback();
