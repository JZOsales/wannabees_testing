...
<body>
  <header>
    ...
    <div class="header-nav">
      ...
      <button class="nav-btn guide-btn" onclick="location.href='customer_guide.php'" title="Guide">
  ...
  <div id="billModal" class="modal">
    <div class="modal-content" style="max-width: 650px;">
      ...
      <div class="modal-footer">
        ...
        <button class="modal-btn btn-confirm" onclick="requestPayment()" id="requestPaymentBtn">
...
<?php endif; ?>
    ...
    async function confirmExtend() {
      ...
      try {
        ...
        const res = await fetch('extend_time.php', {
          method: 'POST',
          body: formData
        });
    ...
    async function confirmOrder() {
      ...
      try {
        ...
        const res = await fetch('add_order.php', {
          method: 'POST',
          body: formData
        });
...
setInterval(async () => {
  try {
    // Update bill
    const billRes = await fetch(`get_bill.php?rental_id=${rentalId}`);
...
function updateOrderMonitor(orders) {
  ...
  if (!orders || orders.length === 0) {
    track.innerHTML = '<div class="empty-orders"><i class="fas fa-utensils"></i><p>No orders yet. Browse the menu below!</p></div>';
    return;
  }
  
  let html = '';
  orders.forEach(order => {
    const items = order.product_name + ' x' + order.quantity;
    const total = parseFloat(order.price || 0) * parseInt(order.quantity || 0);
    html += `
      <div class="order-card">
        <div class="order-header">
          <span class="order-id">Order #${order.order_id}</span>
          <span class="order-status status-${order.status || 'NEW'}">${order.status || 'NEW'}</span>
        </div>
        <div class="order-items">${items}</div>
        <div class="order-time">
          <i class="fas fa-clock"></i> ${new Date(order.ordered_at).toLocaleTimeString('en-US', {hour: 'numeric', minute: '2-digit'})}
...
async function openBillModal() {
  ...
  try {
    const res = await fetch(`get_bill.php?rental_id=${rentalId}`);
    const data = await res.json();
    ...
    if (data.success && data.bill) {
      ...
      if (bill.is_paid == 1) {
        ...
        `;
        document.getElementById('requestPaymentBtn').style.display = 'block';
      }
...
async function requestPayment() {
  if (!confirm('🔔 Request staff to process payment?\n\nA staff member will be notified to come to your room to process your payment.')) {
    return;
  }
  ...
  try {
    ...
    const res = await fetch('request_payment.php', {
      method: 'POST',
      body: formData
    });
    ...
    if (data.success) {
      alert('✅ Payment Request Sent!\n\nStaff has been notified and will come to your room shortly to process your payment.\n\nThank you!');
      closeBillModal();
    } else {
      alert('❌ Error: ' + (data.error || 'Unable to request payment'));
