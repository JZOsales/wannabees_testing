...
if (!isset($_SESSION['user_id']) || !in_array(intval($_SESSION['role_id']), [1,2])) { http_response_code(403); echo json_encode(['success'=>false,'error'=>'Forbidden']); exit; }

$order_id = isset($_POST['order_id']) ? intval($_POST['order_id']) : 0;
$status = isset($_POST['status']) ? strtoupper(trim($_POST['status'])) : '';
$valid = ['NEW','PREPARING','READY','DELIVERED'];
...
try {
    ...
    $allowed = ['NEW' => ['PREPARING'], 'PREPARING' => ['READY'], 'READY' => ['DELIVERED']];

    if ($user_role !== 1) {
        if ($assigned === null || $assigned !== $user_id) throw new Exception('You must claim the order first or you are not assigned this order');
